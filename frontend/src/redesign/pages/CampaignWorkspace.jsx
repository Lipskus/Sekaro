...
export default function CampaignWorkspace(){
 ...
 const check=useCallback(async()=>{setBusy(true);setError('');try{const r=await api.get(`/campaigns/${id}/preflight`);setReport(r);return r;}catch(e){setError(e);return null;}finally{setBusy(false);}},[id]);
 ...
 const campaignState=c.paused?(report?.ready?['green','Gotowa do startu']:['amber','Wstrzymana']):['blue','Aktywna'];
 ...
 return <div className="sk-page sk-campaign-page">
  ...
  <nav className="sk-tabs" aria-label="Sekcje kampanii">{tabs.map(([key,label])=><Link className={`sk-tab ${tab===key?'active':''}`} to={'#'+key} key={key}>{label}</Link>)}</nav>
  {tab==='overview'?<>
   ...
   <div className="sk-campaign-grid"><div>
    <Panel title="Harmonogram wysyłki" icon="calendar"><div className="sk-schedule-form">
     ...
     <dl className="sk-mailbox-limits">{[['Dzienny limit na skrzynkę',first?.max_emails_per_day,'wiadomości'],['Godzinowy limit na skrzynkę',first?.max_emails_per_hour||'—','wiadomości'],['Minimalny odstęp',first?.wait_minutes_between,'minut'],['Losowe opóźnienie',first?'0–'+(first.max_jitter_seconds/60):'—','minut']].map(([label,value,unit])=><div key={label}><dt>{label}</dt><dd><strong>{value??'—'}</strong> {unit}</dd></div>)}</dl>
