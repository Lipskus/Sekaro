...
export default function Shell({children}){
 ...
 return <div className="sk-shell">
  ...
  <aside className={`sk-sidebar ${menu?'is-open':''}`}>
   ...
   <div className="sk-sidebar-bottom"><Link to="/system-health" className="sk-system-card"><div><strong>System</strong><Badge tone={state[1]} dot>{state[0]}</Badge></div><dl><dt>Wersja</dt><dd>0.5.7</dd><dt>Środowisko</dt><dd>{isProduction?'Produkcja':'Testowe'}</dd><dt>Dysk</dt><dd>{diskFree}</dd></dl><progress className="sk-storage-progress" data-tone={diskTone} max="100" value={diskUsed} aria-label="Wykorzystanie dysku" title={storage?.available?`Wykorzystano ${diskUsed}% przestrzeni`:'Brak danych o dysku'}/></Link><Link to="/settings" className="sk-selfhost"><Icon name="server" size={31}/><div><strong>Self-hosted</strong><small>Twoje dane. Twoje zasady.</small></div></Link></div>
  </aside>
