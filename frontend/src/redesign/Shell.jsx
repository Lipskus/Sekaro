...
export default function Shell({children}){
 const {user,logout}=useAuth();const {themePreference,setThemePreference}=useDarkMode();const {overallStatus,rawData,loading:healthLoading,fetchError:healthError}=useSystemHealth();const {count}=useNotifications();const {isProduction}=useAppMode();const {language,setLanguage,languages}=useLanguage();
 ...
 const state=healthError?['Brak danych','neutral']:healthLoading&&overallStatus==='unknown'?['Sprawdzanie…','neutral']:({ok:['Wszystko działa','green'],warning:['Wymaga uwagi','amber'],error:['Wykryto problem','red'],unknown:['Stan nieznany','neutral']})[overallStatus]||['Stan nieznany','neutral'];
 return <div className="sk-shell">
  ...
  <aside className={`sk-sidebar ${menu?'is-open':''}`}>
   ...
   <div className="sk-sidebar-bottom"><Link to="/system-health" className="sk-system-card"><div><strong>System</strong><Badge tone={state[1]} dot>{state[0]}</Badge></div><dl><dt>Wersja</dt><dd>0.5.6</dd><dt>Środowisko</dt><dd>{isProduction?'Produkcja':'Testowe'}</dd><dt>Dysk</dt><dd>{diskFree}</dd></dl><progress className="sk-storage-progress" data-tone={diskTone} max="100" value={diskUsed} aria-label="Wykorzystanie dysku" title={storage?.available?`Wykorzystano ${diskUsed}% przestrzeni`:'Brak danych o dysku'}/></Link><Link to="/settings" className="sk-selfhost"><Icon name="server" size={31}/><div><strong>Self-hosted</strong><small>Twoje dane. Twoje zasady.</small></div></Link></div>
  </aside>
